...
# H-PPO（多分辨率采样策略）
parser.add_argument('--use_hppo', action='store_true', help='是否启用多分辨率采样的H-PPO策略')
parser.add_argument('--hppo_eps_clip_d', type=float, default=0.2)
parser.add_argument('--hppo_eps_clip_c', type=float, default=0.2)
parser.add_argument('--hppo_coeff_critic', type=float, default=0.5)
parser.add_argument('--hppo_coeff_ent_d', type=float, default=1e-3)
parser.add_argument('--hppo_max_norm_grad', type=float, default=5.0)
parser.add_argument('--hppo_init_log_std', type=float, default=-2.0)
parser.add_argument('--hppo_penalty_alpha_d', type=float, default=0.0)
parser.add_argument('--hppo_penalty_alpha_c', type=float, default=0.0)
parser.add_argument('--hppo_target_kl_d', type=float, default=None)
parser.add_argument('--hppo_target_kl_c', type=float, default=None)
parser.add_argument('--hppo_policy_decay', type=str, default='none', choices=['none','down'])
parser.add_argument('--hppo_actor_d_lr', type=float, default=1e-3)
parser.add_argument('--hppo_actor_c_lr', type=float, default=1e-3)
parser.add_argument('--hppo_critic_lr', type=float, default=1e-3)
parser.add_argument('--hppo_minibatch_size', type=int, default=512)
parser.add_argument('--hppo_update_nums', type=int, default=5)
parser.add_argument('--hppo_gamma', type=float, default=0.99)
parser.add_argument('--hppo_lam', type=float, default=0.95)
parser.add_argument('--hppo_reward_with_distortion', action='store_true', help='奖励是否包含失真正则项（与训练目标完全对齐）')
